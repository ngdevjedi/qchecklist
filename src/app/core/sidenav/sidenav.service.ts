...
export class SidenavService {
  ...
  constructor() {
    ...
    let dashboard = menu.addItem('Dashboard', 'dashboard', '/', 1, '16', '#7986CC');

    let components =  menu.addItem('Components', 'layers', null, 2);
    ...
    let maps =  menu.addItem('Maps', 'map', null, 5, '3', '#4CAF50');
    menu.addSubItem(maps, 'Google Maps', '/maps/google-maps', 1);

    let icons = menu.addItem('Material Icons', 'grade', '/icons', 6);

    let customPages = menu.addItem('Custom Pages', 'web', null, 7);
    ...
    let editor = menu.addItem('WYSIWYG Editor', 'format_shapes', '/editor', 8, 'NEW', '#3F51B5');

    let dynamicMenu = menu.addItem('Dynamic Menu', 'extension', '/dynamic-menu', 9);

    let multiLevel = menu.addItem('Multi-Level Menu', 'menu', null, 10);
